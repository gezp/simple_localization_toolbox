...
bool LidarLocalization::init_config(const std::string & config_path, const std::string & data_path)
{
  ...
  // init filter
  using BoxFilter = slt_common::BoxFilter;
  ...
  // init buffer
  gnss_odom_buffer_ = std::make_shared<slt_common::OdomDataBuffer>(1000);
  // print info
...
bool LidarLocalization::add_gnss_data(const slt_common::GnssData & gnss_data)
{
...
bool LidarLocalization::add_gnss_odom(const slt_common::OdomData & gnss_odom)
{
...
slt_common::OdomData LidarLocalization::get_current_odom()
{
