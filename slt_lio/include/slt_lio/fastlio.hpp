...
// FAST-LIO style tightly coupled lidar-inertial odometry: owns the eskf, the imu/lidar
// buffers and the initialization, registers against a key frame based local map
// (sliding window + voxel rebuild) with a pcl kdtree.
// reference: FAST-LIO (https://github.com/hku-mars/FAST_LIO)
...
// get_imu_nav_state()
class Fastlio : public LioInterface
{
public:
  using PointCloudPtr = pcl::PointCloud<pcl::PointXYZ>::Ptr;

  explicit Fastlio(const YAML::Node & config);
  ~Fastlio() override;
  void set_extrinsic(
...
private:
  ...
  // key frame based sliding window local map
  struct Frame
  {
    PointCloudPtr point_cloud;
  };
  std::shared_ptr<slt_common::VoxelFilter> local_map_filter_;
  PointPlaneCorrespondenceSearch::Config correspondence_config_;
